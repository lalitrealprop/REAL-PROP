...
export default function Navbar({ isAdmin }: { isAdmin: boolean }) {
  ...
  const projects = [
    ...
    { name: 'ACE ARTE', href: '/projects/ace-arte-sector-150-noida' },
  ];
  const locations = [
  ...
  return <>
    ...
    <nav className={cn('fixed left-0 right-0 z-50 transition-all duration-300 flex items-center', isSVGProject ? 'top-9 h-16' : 'top-0 h-16', scrolled ? 'bg-white shadow-md' : isSVGProject ? 'bg-[#08090c]/80 backdrop-blur-md' : 'bg-transparent')}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        <Link to={isSVGProject ? '/projects/svg-central-square' : '/'} className="flex items-center shrink-0 relative z-[80]" aria-label="REAL PROP"><img src="https://instasize.com/api/image/87e2fdb8828fd9cdfa4566774e9ba73c587ea743872310c777de048c2b9dd4b1.jpeg" alt="REAL PROP Logo" className="h-11 w-11 sm:h-12 sm:w-12 rounded-full object-contain border-2 border-red-600 shadow-sm block opacity-100" /></Link>
        <div className="hidden md:flex items-center space-x-6 lg:space-x-7">
          {isSVGProject ? <>
            <Link to="/" onClick={()=>{setIsOpen(false);setShowProjects(false);setShowLocations(false);}} className={cn('px-4 py-2 rounded-full border text-xs font-black whitespace-nowrap transition-colors', scrolled?'border-[#d7ad5a] text-[#9b762e] hover:bg-[#d7ad5a] hover:text-black':'border-[#d7ad5a]/70 text-[#d7ad5a] hover:bg-[#d7ad5a] hover:text-black')}>← REAL PROP HOME</Link>
      ...
      <AnimatePresence>{isOpen && <motion.div initial={{opacity:0,y:-15}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-15}} className={cn('absolute left-0 right-0 bg-white shadow-xl md:hidden py-4 px-4 flex flex-col space-y-3 overflow-y-auto max-h-[80vh]', isSVGProject ? 'top-16' : 'top-16')}>
        {isSVGProject ? <>
          <Link to="/" onClick={()=>{setIsOpen(false);setShowProjects(false);setShowLocations(false);}} className="text-left text-lg font-black text-[#9b762e]">← REAL PROP HOME</Link>
